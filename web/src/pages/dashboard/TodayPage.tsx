    ...
    <>
      ...
      <div className="pomo-page-header">
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button className="pomo-btn" onClick={() => navigate('/dashboard/history?view=week')}>
            <i className="ti ti-calendar" /> Week view
          </button>
          <button className="pomo-btn" onClick={() => navigate('/dashboard/history?view=month')}>
            <i className="ti ti-calendar-month" /> Month view
          </button>
      ...
      {showReport && (
        <ReportModal
          title="Daily report"
          filename={`pomodoso-report-${data.date}.md`}
          content={buildReport(data, `${dayName}, ${dateStr}`)}
