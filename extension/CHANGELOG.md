...
- **New: Today sorts itself as work moves** — Marking a task done or cancelled drops it to the bottom of its section; marking it in progress lifts it to the top. Delayed and to-do stay exactly where you dragged them. Priorities and Today's tasks each sort on their own, so finishing a priority never pushes it out of Priorities. On by default, with an On/Off switch in Settings → General for anyone who'd rather keep a purely manual order.
- **Fix: "Use template" no longer starts you on an empty screen** — The sample tasks were created, then the workspace migration moved them to their real workspace while the app kept pointing at the old placeholder one, so Today and the Backlog both rendered empty and only the habits showed up. A new install now opens on "All workspaces", which shows your data wherever it lives.
...
- **Fix: dragging a task no longer just selects its text** — Rows are mostly text and the drag sensor doesn't suppress the browser's default, so pressing and dragging started a text selection instead of picking the row up.
- **Reorder tasks by dragging, everywhere** — Today's priorities, Today's tasks and the Backlog all reorder by drag: pick a row up anywhere on it. Dragging a task into Priorities respects the limit set in Settings instead of a hardcoded three. Works per workspace and under "All workspaces", and the Backlog order syncs across devices.
...
- **Recurring tasks: carry over if missed** — New per-task toggle in the recurrence editor (default **on**). When on, a recurring task that wasn't done stays in Today until you complete or cancel it (one instance, no stacking — it's the same task object) instead of disappearing at the day rollover. Turn it off to keep the old "gone next day, back on the next scheduled occurrence" behavior. Stored as `carryOver` on `RecurrenceRule` (absent = carry over).
- **Recurring tasks: repeat every N** — Recurrence now supports an interval (`Every [N] days/weeks/months/years`), so biweekly ("every 2 weeks"), "day 1 every 2 months", etc. are possible. New optional `interval` on `RecurrenceRule` (default 1, travels in `task.extra`); occurrence math (`shouldOccurOn`/`shouldBeInTodayNow`) counts periods from the start date, and labels read "Every 2 weeks on …". Shown on the web Tasks page too.
...
- **Clear sign-up path on the landing** — The landing header now has a visible **Sign up** button (→ `/login?mode=signup`, opens the create-account form directly) next to Sign in, the `/login` toggle to create an account is more prominent ("Create an account", accent), and the logged-out pricing CTAs (Get Pro / Get Lifetime) route through sign-up then billing. Previously the only paths were "Sign in" and "Add to Chrome", so new users couldn't obviously create a web account.
